#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#Title - Approach 2: Random Forest with PCA FARM SPLIT 
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# This code is for Random Forest Analysis (Binary Classification Method) for Lameness Detection
# in Dairy Cows based on Accelerometer Data using PCA-Transformed Features.

# Prior Steps:
# - Principal Component Analysis (PCA) was performed on the collected accelerometer data.
# - Input Files:
#   1. AfterPCA_AllCowsX.csv: Contains PCA-transformed features
#   2. CombConorAll.csv: Contains the binary outcome (0/1) for lameness classification.
# - Ensure the dependent variable is binary (0/1) and correctly formatted.

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Steps in the Code:
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Step 1: Load Required Libraries.
# Step 2: Load and Prepare Data for RF Analysis.
# Step 3: Set Up Cross-Validation (Farm-fold) with Performance Metrics Storage.
# Step 4: Train and Test the Random Forest Model for Each Fold.
# Step 5: Evaluate and Save Model Performance Metrics.
# Step 6: Generate and Save Calibration Plot.
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
AllCowsData_NRX <- AfterPCA_AllCowsX %>%
...
write_xlsx(df, "A2_PCA_RF_FS3.xlsx")
...
# Load Model Output
CR_A2_PCA_RF_FS <- read_excel("A2_PCA_RF_FS3.xlsx")  
CR_A2_PCA_RF_FS$Observed_Outcome <- as.numeric(CR_A2_PCA_RF_FS$Observed_Outcome)
CR_A2_PCA_RF_FS <- CR_A2_PCA_RF_FS %>% select(Observed_Outcome, Predicted_Probabilities)
names(CR_A2_PCA_RF_FS) <- c("event", "predicted")

# Open a PNG device to save the plot
png(filename = "C_A2_PCA_RF_FS.png", width = 800, height = 600)
# Generate and display the calibration plot
calibration_plot <- print_calibration_plot(CR_A2_PCA_RF_FS, bins = 10)
