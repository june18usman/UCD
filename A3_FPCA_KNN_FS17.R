...
for (i in seq_along(folds)) {
  ...
  predicted_classes <- ifelse(predicted_probabilities > 0.25, 1, 0)  # Apply threshold
...
write_xlsx(df, "A3_FPCA_KNN_FS17.xlsx")
...
# Load Model Output
CR_A3_FPCA_KNN_FS <- read_excel("A3_FPCA_KNN_FS17.xlsx")
CR_A3_FPCA_KNN_FS$Observed_Outcome <- as.numeric(CR_A3_FPCA_KNN_FS$Observed_Outcome)
CR_A3_FPCA_KNN_FS <- CR_A3_FPCA_KNN_FS %>% select(Observed_Outcome, Predicted_Probabilities)
names(CR_A3_FPCA_KNN_FS) <- c("event", "predicted")

# Open a PNG device to save the plot
png(filename = "C_A3_FPCA_KNN_FS.png", width = 800, height = 600)
# Generate and display the calibration plot
calibration_plot <- print_calibration_plot(CR_A3_FPCA_KNN_FS, bins = 15)
