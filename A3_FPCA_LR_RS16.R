...
# Define grid search for logistic regression (tuning 'lambda' or regularization)
grid <- expand.grid(
  alpha = c(0, 0.5, 1),  # L1, L2, and Elastic Net (mixing between L1 and L2)
  lambda = seq(0.01, 1, length = 20)  # Regularization parameter range
)
...
for (i in seq_along(folds)) {
  ...
  # Train the Logistic Regression model using grid search for tuning parameters
  model <- train(
    High ~ .,                      # Use all features to predict 'High'
    data = train_data,              # Training data
    method = "glmnet",              # Elastic net regularization (Logistic Regression)
    family = "binomial",            # Logistic regression (binomial)
    trControl = train_control,      # Cross-validation settings
    tuneGrid = grid                 # Grid of tuning parameters
  )
...
write_xlsx(df, "A3_FPCA_LR_RS16.xlsx")
...
# Load Model Output
CR_A3_FPCA_LR_RS <- read_excel("A3_FPCA_LR_RS16.xlsx")  
CR_A3_FPCA_LR_RS$Observed_Outcome <- as.numeric(CR_A3_FPCA_LR_RS$Observed_Outcome)
CR_A3_FPCA_LR_RS <- CR_A3_FPCA_LR_RS %>% select(Observed_Outcome, Predicted_Probabilities)
names(CR_A3_FPCA_LR_RS) <- c("event", "predicted")

# Open a PNG device to save the plot
png(filename = "C_A3_FPCA_LR_RS.png", width = 800, height = 600)
# Generate and display the calibration plot
calibration_plot <- print_calibration_plot(CR_A3_FPCA_LR_RS, bins = 15)
