#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Title - Approach 1: Random Forest without reduction [Farm-Fold]
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# This code is for Random Forest Analysis (Binary Classification Method) for Lameness Detection
# in Dairy Cows based on Accelerometer data 

# - Input Files:
#   1. AllCowsDataX.csv: Contains features (383 rows for cows, 20,000 observations/30 sec).
...
# Step 2: Load and Prepare Data for Random Forest Analysis.
# Step 3: Set Up Farm based validation with Performance Metrics Storage.
...
AllCowsData_NRX <- AllCowsDataX



# Add 'High' column (lameness status)
AllCowsData_NRX <- cbind(AllCowsData_NRX, CombConorAll$High)
...
write_xlsx(df, "A1_NR_RF_FS1.xlsx")
...
CR_A1_NR_RF_FS <- read_excel("A1_NR_RF_FS1.xlsx") 
CR_A1_NR_RF_FS$Observed_Outcome <- as.numeric(CR_A1_NR_RF_FS$Observed_Outcome)
CR_A1_NR_RF_FS <- CR_A1_NR_RF_FS %>% select(Observed_Outcome, Predicted_Probabilities)
names(CR_A1_NR_RF_FS) <- c("event", "predicted")

# Open a PNG device to save the plot
png(filename = "C_A1_NR_RF_FS.png", width = 800, height = 600)
# Generate and display the calibration plot
calibration_plot <- print_calibration_plot(CR_A1_NR_RF_FS, bins = 10)
