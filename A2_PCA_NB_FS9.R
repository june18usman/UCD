...
setwd("C:/Users/musman/Documents/R Files for Final Results/FINAL CODE") #NEW
...
for (i in seq_along(folds)) {
  ...
  predicted_probabilities <- predict(model, newdata = test_data, type = "raw")[, "1"]  # Probabilities for class "1"
  predicted_classes <- ifelse(predicted_probabilities > 0.5, 1, 0)  # Apply threshold (default threshold is 0.5)
...
write_xlsx(df, "A2_PCA_NB_FS9.xlsx")
...
# Load Model Output
CR_A2_PCA_NB_FS <- read_excel("A2_PCA_NB_FS9.xlsx") 
CR_A2_PCA_NB_FS$Observed_Outcome <- as.numeric(CR_A2_PCA_NB_FS$Observed_Outcome)
CR_A2_PCA_NB_FS <- CR_A2_PCA_NB_FS %>% select(Observed_Outcome, Predicted_Probabilities)
names(CR_A2_PCA_NB_FS) <- c("event", "predicted")

# Open a PNG device to save the plot
png(filename = "C_A2_PCA_NB_FS.png", width = 800, height = 600)
# Generate and display the calibration plot
calibration_plot <- print_calibration_plot(CR_A2_PCA_NB_FS, bins = 15)
