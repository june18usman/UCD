...
write_xlsx(df, "A2_PCA_LR_FS5.xlsx")
...
# Load Model Output
CR_A2_PCA_LR_FS <- read_excel("A2_PCA_LR_FS5.xlsx") 
CR_A2_PCA_LR_FS$Observed_Outcome <- as.numeric(CR_A2_PCA_LR_FS$Observed_Outcome)
CR_A2_PCA_LR_FS <- CR_A2_PCA_LR_FS %>% select(Observed_Outcome, Predicted_Probabilities)
names(CR_A2_PCA_LR_FS) <- c("event", "predicted")

# Open a PNG device to save the plot
png(filename = "C_A2_PCA_LR_FS.png", width = 800, height = 600)
# Generate and display the calibration plot
calibration_plot <- print_calibration_plot(CR_A2_PCA_LR_FS, bins = 10)
