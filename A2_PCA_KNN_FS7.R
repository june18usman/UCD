...
setwd("C:/Users/musman/Documents/R Files for Final Results")
...
print(length(CombConorAll$High))
print(nrow(AfterPCA_AllCowsX))  
...
for (i in seq_along(folds)) {
  ...
  # Train KNN model with fixed k
  knn_model_final <- knn3(
    High ~ ., 
    data = train_data, 
    k = 7
  ) 
  ...
  # predicted_probabilities <- predict(knn_model_final, test_data, type = "prob")
  predicted_probabilities <- predict(knn_model_final, test_data, type = "prob")[, "1"]
  predicted_classes <- ifelse(predicted_probabilities > 0.5, 1, 0)  # Apply threshold
  
  # Confusion matrix
  conf_matrix <- confusionMatrix(as.factor(predicted_classes),as.factor(test_split$labels), positive = "1")
  
  print(dim(test_data))
  print(length(predicted_probabilities))
  print(length(test_split$labels))
...
write_xlsx(df, "A2_PCA_KNN_FS7.xlsx")
...
# Load Model Output
CR_A2_PCA_KNN_FS <- read_excel("A2_PCA_KNN_FS7.xlsx") 
CR_A2_PCA_KNN_FS$Observed_Outcome <- as.numeric(CR_A2_PCA_KNN_FS$Observed_Outcome)
CR_A2_PCA_KNN_FS <- CR_A2_PCA_KNN_FS %>% select(Observed_Outcome, Predicted_Probabilities)
names(CR_A2_PCA_KNN_FS) <- c("event", "predicted")

# Open a PNG device to save the plot
png(filename = "C_A2_PCA_KNN_FS.png", width = 800, height = 600)
# Generate and display the calibration plot
calibration_plot <- print_calibration_plot(CR_A2_PCA_KNN_FS, bins = 15)
